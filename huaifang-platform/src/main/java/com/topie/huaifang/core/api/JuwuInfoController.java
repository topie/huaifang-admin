 ...
 */
@Controller
@RequestMapping("/api/core/juwuInfo")
public class JuwuInfoController {
    ...
    public Result list(JuwuInfo juwuInfo,
        ...
        PageInfo<JuwuInfo> pageInfo = iJuwuInfoService.selectByFilterAndPage(juwuInfo, pageNum, pageSize);
    ...
    public Result formItems() {
        List<FormItem> list = iCommonQueryService.selectFormItemsByTable("d_juwu_info");
        return ResponseUtil.success(list);
    }

    @RequestMapping(value = "/insert", method = RequestMethod.POST)
    @ResponseBody
    public Result insert(JuwuInfo juwuInfo) {
        juwuInfo.setStatus("上线");
        int result = iJuwuInfoService.saveNotNull(juwuInfo);
        return result > 0 ? ResponseUtil.success() : ResponseUtil.error();
    }

    @RequestMapping(value = "/update", method = RequestMethod.POST)
    @ResponseBody
    public Result update(JuwuInfo juwuInfo) {
        iJuwuInfoService.updateNotNull(juwuInfo);
